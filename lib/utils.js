...
const pipe = tasks => pPipe(...tasks);
exports.seq = tasks =>
  pipe(tasks.map(task => ctx => Promise.resolve(task(ctx)).then(() => ctx)));
...
exports.ite = (cond, truthy, falsy) => ctx =>
  Promise.resolve(cond(ctx)).then(res =>
    res ? invokeOrReturn(truthy, ctx, true) : invokeOrReturn(falsy, ctx, false)
  );
